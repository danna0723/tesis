...
Por otra parte, la metodologia de implementar el uso de lags, tambien se tomo del repositorio "Demand Forecasting \& Inventory Optimization for Consumer Products" (https://github.com/dhruvi002/demand-forecast-inventory-optimizer) y se adapto a las necesidades del sistema Thary, agregando la codificacion cíclica de los meses y la inclusion de features adicionales como el inventario y el precio.
...
Otra estrategia que se tomo del repositorio "Demand Forecasting \& Inventory Optimization for Consumer Products", fue el uso de las estadísticas móviles. Estas aportan al sistema ya que consisten en un resumen de varios meses juntos. Mientras que los rezagos solo representan cuanto se vendio exactamente en un mes, las estadísticas móviles son un resumen de varios meses juntos.
...
Por otra parte, para la construccion de la etapa de Features tambien se tomo en cuenta el repositorio en Kaggle "Inventory Demand Forecast (XGBoost Reg)" (https://www.kaggle.com/code/ksmooi/inventory-demand-forecast-xgboost-reg). De este repositorio, se tomo el uso de el promedio del producto, pero esta vez, calculados en base a todo el historial completo y no solo en base a los ultimos 3 o 6 meses. 
...
En esta parte, se implemento otra idea del repositorio ``Demand Forecasting
\textbackslash \& Inventory Optimization for Consumer Products'' para poder descartar las filas sin historial suficiente para tener los lags. Para esto, se define una lista con nueve nombres de las columnas obligatiorias para poder entrenar, con (\texttt{(dropna(subset=feature\_history))}) se revisan las 4 columnas y elimina cualquier fila que tenga un espacio vacio, posteriormente el resultado se guarda en (\texttt{(df\_model)}), que es la tabla final que se usara para entrenar el modelo.
