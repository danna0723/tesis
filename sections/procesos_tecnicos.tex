...
Conceptualmente, el modelo de datos del sistema Thary utiliza un modelo no relacional para guardar la informacion mediante el uso de archivos JSON y CSV organizados en carpetas para cada empresa. Las cinco entidades principales son: Usuario, pedido, registro de consumo historico, recomendacion de reposicion y comportamiento del producto. Para garantizar que el sistema permita que cada organizacion tenga su propio conjunto de datos independiente, (tal como se describio en la seccion del diseño de arquitectura), se relaciono todas las entindades mediante el identificador de producto y el identificador de la organizacion \texttt{empresa\_id}.

Logica y Fisicamente, la entidad Usuario se almacena en un unico archivo \texttt{usuarios.json}, en el que al registrarse un usuario, se guardan datos del usaurio como nombre, nombre de usuario, correo electronico, la contraseña cifrada, el rol, ya sea admin o empleado, y el id de la empresa a la que pertenece. La entidad Pedido también se almacena de forma independiente en el archivo \texttt{pedidos.json}; este archivo está separado de los resultados del procesamiento y es único para cada empresa, así se garantiza que cuando se cargue un nuevo historial, no se elimine el registro de los pedidos ya confirmados. Por otro lado, las entidades de registro de consumo historico, recomendacion de reposicion y comportamiento del producto son generadas despues de que el sistema procese el archivo CSV que fue cargado por el administrador. Estos resultados se guardan en los archivos \texttt{segmentacion\_productos.csv} y \texttt{reorder\_table.csv}, en una carpeta de resultados distinta para cada empresa.
